...
async function dataLoaded(data) {
  ...
  if (
    typeof data.frequencySeekNewQuestions === "undefined" ||
    data.frequencySeekNewQuestions === null
  ) {
  ...
  // Load locale
  if (
    typeof data.chooseLanguage === "undefined" ||
    data.chooseLanguage === null
  ) {
    locale = ["en-US"];
    browser.storage.local.set({
  ...
  if (
    typeof data.showNotifications === "undefined" ||
    data.showNotifications === null
  ) {
  ...
  if (
    typeof data.chooseProduct === "undefined" ||
    data.chooseProduct === null
  ) {
    product = ["Firefox"];
    browser.storage.local.set({
      chooseProduct: product,
    ...
    removedProducts.map((removedProduct) => {
      if (product.includes(removedProduct)) {
        product = product.filter(
          (productName) => productName !== removedProduct
        );
        ...
        browser.storage.local
          ...
          .then(() => {
            console.log(
              `${removedProduct} successfully removed from chooseProduct.`
            );
          })
          .catch((error) => {
...
function loadRequest(request) {
  ...
  for (let question of availableQuestions) {
    const listIndex = questionList.findIndex((q) => {
  ...
  if (responseSUMO.results.length > 0)
    removeOld(
      responseSUMO.results,
      responseSUMO.results[0].product,
      responseSUMO.results[0].locale
    );
...
function updateQuestionCount() {
  ...
  if (numberOfQuestionsOpened >= 2) {
    browser.browserAction.setTitle({
      title:
        numberOfQuestionsOpened +
        browser.i18n.getMessage("more_than_one_question_without_answer"),
    });
  } else if (numberOfQuestionsOpened == 1) {
    browser.browserAction.setTitle({
      title:
        numberOfQuestionsOpened +
        browser.i18n.getMessage("one_question_without_answer"),
    });
  } else {
    browser.browserAction.setTitle({
      title: "",
...
function removeOld(questions, productToCheck, localeToCheck) {
  ...
  const productList = product.map((v) => {
    return v.toLowerCase();
  });
  const localeList = locale.map((v) => {
    return v.toLowerCase();
  });
  const validList = [];
  const removeList = [];
  ...
  for (let question of questionList) {
    const inScope =
      question.product.toLowerCase() == productToCheck.toLowerCase() &&
      question.locale.toLowerCase() == localeToCheck.toLowerCase();
    const isPossible =
      productList.includes(question.product.toLowerCase()) &&
      localeList.includes(question.locale.toLowerCase());
    ...
    if (
      !(
        (!inScope && isPossible) ||
        (isPossible && onList && !isExpired && !isTaken)
      )
    ) {
      question.show = false;
  ...
  for (let question of removeList) {
    browser.runtime.sendMessage({
...
function showNotification(questions) {
  ...
  if (num == 1) {
    ...
    message +=
      ...
      (num - 1) +
      " " +
      browser.i18n.getMessage("notification_message_more");
  }
  ...
  browser.notifications.create({
    ...
    iconUrl: browser.runtime.getURL(
      "/res/products/" + questions[0].product + ".png"
    ),
    title: title,
    message: message,
  });
...
function markAsRead(id) {
  ...
  const index = questionList.findIndex((q) => {
    return q.id == id;
  });
